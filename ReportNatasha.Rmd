...
ZO_data_clean <- select(ZO_data,one_of(c("Colony.Prefix","Genotype2","Gender",XrayVariableList_c)))
...
cond.test<-function (row) 
{
  ## deformed in KO (F and M)
  xx1 <- row$n111
  xx2 <- row$n112
  ## deformed in KO and WT(F and M)
  mm1 <- row$n111 + row$n121 # = n1.1
  mm2 <- row$n112 + row$n122 # = n1.2
  ## not deformed in KO and WT (F and  M)
  nn1 <- row$n211 + row$n221 # = n2.1
  nn2 <- row$n212 + row$n222 # = n2.2
  ...
  x <- dhyper(x = AA1:BB1, m = mm1, n = nn1, k = kk1)
  y <- dhyper(x = AA2:BB2, m = mm2, n = nn2, k = kk2)
  pdf <- convolve(x,rev(y),type = "open")
  supp <- (AA1+AA2):(BB1+BB2)
  alpha_star <- rev(pdf)[1]
  index_pv <- which(rev(supp)==(xx1+xx2))
  p <- sum(rev(pdf)[1:index_pv])
  f_n <- rev(pdf)[index_pv]
  kappa <- sum(pdf==f_n)   
...
p_adjust <- function(p,alpha_star,f_n,kappa,alpha = 0.05,lambda = 0.05,seed2 = 1, Kvec = NULL, rr=1000)
{
...
tbl_p     <- tbl_222_sorted %>% rowwise() %>%  do(cond.test(.)) 
...
tbl_K <- select(tbl_pa_each,fam_size.3)
B=1000
for (j in 1:B)
{
  tbl_222_sim <- 
  ...
  tbl_pa_each_sim     <- cbind(tbl_p,tbl_K) %>% group_by(Outcome) %>% 
  do(p_adjust(.$p,.$alpha_star,.$f_n,.$kappa,alpha = 0.05,Kvec = .$fam_size.3,seed2 = j,rr = 500))
