...
{
  ## deformed in KO (F and M)
  xx1 <- df$n111
  xx2 <- df$n112
  ## deformed in KO and WT(F and M)
  ...
  ## in KO deformed and not deformed(F and M)
  kk1 <- df$n.11
  kk2 <- df$n.12
...
{ b=1
  ...
  tbl_pa_each_sim <- tbl_p %>% tbl_df() %>% cbind(.,select(tbl_pdf,alpha_star),select(tbl_K,fam_size.3)) %>%
    group_by(Outcome) %>% do(p_adjust(.$p,.$alpha_star,.$f_n,.$kappa,alpha = 0.05,
                                      Kvec = .$fam_size.3,index_for_seed = index_seed+b*max(index_seed)))
  sim_result_list[[b]] <- tbl_pa_each_sim
}

save(sim_result_list,file="sim_result_list.RData")

get_n_Rej <- function(x) as.data.frame(t(colSums(x<=0.05)))
# summarize results:
nRej  <- lapply(sim_result_list, FUN = function(x) {x %>%
    group_by(Outcome) %>% do(get_n_Rej(.[-c(1,20:25)]))})
fwe  <- lapply(nRej,function(x) x>1)
fwer <- Reduce('+', fwe)/length(nRej)
cbind(unique(tbl_K),fwer)[c(9,19,24,41),-c(3:5,7,8,10,11,13,14,16,17,19,20)]
...
B <- 100
# tbl_K <- NULL
sim_result_list_eff <- list()
index_seed <- as.numeric(tbl_222_sorted$Outcome)

for (b in 1:B)
{
  cat(b,"\n")
  for (ii in which_true_alt)
    n111[ii] <- rnoncenhypergeom(50, n = n211 + n221,m = n121 + n111,k = n.11)
  (1,n1 = n_u,n2 = N-n_u,m1 = n_t[ii],psi = OR)
  tbl_222_sim <- 
    tbl_222_sorted %>% mutate(n111 = rhyper(nn = nrow(.),n = n211 + n221,m = n121 + n111,k = n.11),
                              n112 = rhyper(nn = nrow(.),n = n212 + n222,m = n112 + n122,k = n.12))
  tbl_p     <- tbl_222_sim %>% rowwise() %>%  do(cond.test(.)) %>%
    cbind(select(tbl_222_sim,Outcome),.) %>% tbl_df()
  #tbl_p_int <- tbl_222_sim %>% rowwise() %>%  do(mod.zelen.test(.)) 
  
  if (is.null(tbl_K))
  {
    tbl_pa_each_sim     <- tbl_p %>% group_by(Outcome) %>% 
      do(p_adjust(.$p,.$alpha_star,.$f_n,.$kappa,alpha = 0.05,Kvec = NULL,index_for_seed = index_seed))
    tbl_K <- select(tbl_pa_each_sim,fam_size.3)
  }
  else
    tbl_pa_each_sim     <- cbind(tbl_p, select(tbl_K,fam_size.3)) %>% group_by(Outcome) %>%
      do(p_adjust(.$p,.$alpha_star,.$f_n,.$kappa,alpha = 0.05,Kvec = .$fam_size.3,
                  index_for_seed = index_seed+b*max(index_seed)))
  sim_result_list[[b]] <- tbl_pa_each_sim
}
...
# summarize results:
Rej <- lapply(sim_result_list,FUN = function(x) x[-1]<=0.05)

nRej <- lapply(Rej,FUN = function(x) colSums(x))

xx <- Reduce('+', nRej)/length(out)

sim_result_list[[1]]<=0.05
