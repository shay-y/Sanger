...
# pi0 estimation given randomized p-value (Dickhaus 2012):
...
## set seed for reproducibility and get randomized pv:
set.seed(1)
...
pi_0_all   <- Pi_0_est_EU(p,f_n,kappa,in_fam = rep(T,length(p_rand)),lambda = lambda)
...
tb_in_each_0_05 <- cbind(tapply(pa_1[,2],oc_ind,function(x) sum(x<=alpha)),
...
tb_in_each_Gilb <- cbind(tapply(pa_1[,3],oc_ind,function(x) sum(x<=alpha)),
...
save(tb_all,tb_in_each_0_05,tb_in_each_Gilb,file="tables_all.RData")
