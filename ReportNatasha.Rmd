...
fwer_results %>% filter(fam_size.3>20)
```
...
out <- array(dim = c(length(OR),B,2,43,6),dimnames = list(OR=OR,
                                                          b=1:B,
...
{ 
  ...
  R <- tbl_pa_each_sim %>% do(get_n_Rej(.[2:7]))
  V <- tbl_pa_each_sim %>% filter(row_number()>m1)  %>%  do(get_n_Rej(.[2:7]))
  S <- tbl_pa_each_sim %>% filter(row_number()<=m1) %>%  do(get_n_Rej(.[2:7]))
...
load(file="sim_w_effect_results.RData")
load(file="sim_w_effect_results_extras.RData")

fdr <- apply(out[,,1,,,drop=F],c(4,5,1),mean)
pwr <- apply(out[,,2,,,drop=F],c(4,5,1),mean)

F_sizes_alpha <- tbl_pa_each_sim %>% summarise(F_size=unique(fam_size.1)) %>% select(F_size)
F_sizes_Gilb <- tbl_pa_each_sim %>% summarise(F_size=unique(fam_size.2)) %>% select(F_size)

# FDR:
par(mfrow=c(4,4))
for(i in 1:43)
{
  matplot(colnames(fdr[i,c(1,3,5),]),t(fdr[i,c(1,3,5),]),type="l",sub=paste0("Outcome " ,i," |F|=",F_sizes_alpha[i,1]),xlab="Odds Ratio",ylab="fdr")
  if(i %in% c(16,32,43)) dev.copy2pdf(file = paste0("fdr~OR_alpha",i,".pdf") ,height=11,width=11)
}
...
for(i in 1:43)
{
  matplot(colnames(pwr[i,c(1,3,5),]),t(pwr[i,c(1,3,5),]),type="l",sub=paste0("Outcome " ,i," |F|=",F_sizes_alpha[i,1]),xlab="Odds Ratio",ylab="Avg. Power")
  if(i %in% c(16,32,43)) dev.copy2pdf(file = paste0("Power~OR_alpha",i,".pdf") ,height=11,width=11)
}
