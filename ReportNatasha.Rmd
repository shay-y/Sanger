...
p_adjust <- function(p,alpha_star,f_n,kappa,alpha = 0.05,lambda = 0.05)
{
...
# alpha <- 0.05
# tbl_pa <- p_adjust(p = tbl_p$p,
#                    alpha_star = tbl_p$alpha_star,
#                    f_n = tbl_p$f_n,
#                    kappa =tbl_p$kappa)
# 
# tb_all <- tbl_pa %>% select(-starts_with("pi0"),-starts_with("fam")) %>% 
#   summarise_each(funs(sum(.<=alpha)))
# 
# tb_all_named <- as.data.frame(matrix(tb_all,nrow = 3, dimnames = list(c("pa_BH","pa_BH_alpha","pa_Gilb"),
#   c("pa_original_non_adp","pa_original_adp","pa_mid_non_adp","pa_mid_adp"))))
# 
# fam_size <- t(select(tbl_pa,starts_with("fam"))[1,])
# pi0      <- t(select(tbl_pa,starts_with("pi0"))[1,])
# 
