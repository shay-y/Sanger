...
(ZO_data1 <- ZO_data %>%
   ...
   summarise(s = sum(zo,na.rm = T),n = sum(!is.na(zo)) , NAs = sum(is.na(zo))))
...
tbl_222 <- full_join(F_only,M_only,by = c("Outcome","Group.x","Group.y")) %>% 
  ...
  select(Outcome, Group = Group.x,
         ...
         n112 = s.x.y, n.12 = n.x.y, n122 = s.y.y, n.22 = n.y.y) %>% 
  mutate(n211 = n.11 - n111, n221 = n.21 - n121,
         n212 = n.12 - n112, n222 = n.22 - n122)

tbl_222[is.na(tbl_222)] <- 0
...
cbind(tb_rej_in_each_oc_0_05,tb_rej_in_each_oc_0_051)
oo <- order(XrayVariableList_c)
...
unique(XrayVariableList_c)
unique(tbl_222$Outcome)
